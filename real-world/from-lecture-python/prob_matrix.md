...
% --------------------------------------------------------------------------
% Real-world fixture: prob_matrix
% Source:   https://github.com/QuantEcon/lecture-python.myst/blob/main/lectures/prob_matrix.md
% Site:     https://python.quantecon.org/prob_matrix.html
% Captured: 2026-05-27
% Issue:    quantecon-book-theme #387 (RHS TOC overflow + LHS sidebar crowding)
% Notes:    Long page with mixed math, code, tables, and exercises. Exercises
%           RHS "On this page" overflow, LHS sidebar density, and content
%           region spacing across many heading levels.
%
% Sanitization:
%   - {doc} cross-references replaced with placeholder anchors at the bottom
%     of this file. Targets: stats_examples, prob_meaning, navy_captain,
%     lln_clt, multivariate_normal, opt_transport.
%   - {eq} references retained — they resolve within this file.
%   - No {cite} references in source; no references.bib stub needed.
% --------------------------------------------------------------------------
...
This [companion lecture](#placeholder-stats-examples) describes some popular probability distributions and describes how to use Python to sample from them.
...
We also touch on these topics in [prob_meaning](#placeholder-prob-meaning) and [navy_captain](#placeholder-navy-captain).
...
See [lln_clt](#placeholder-lln-clt) for a detailed treatment of both results.
...
```{note}
To generate random draws from the joint distribution $F$, we use the inverse CDF technique described in [this companion lecture](#placeholder-stats-examples).
```
...
```{note}
Please see this [quantecon lecture](#placeholder-multivariate-normal) for more details.
```
...
Couplings are important in optimal transport problems and in Markov processes. Please see this [lecture about optimal transport](#placeholder-opt-transport).
...
The anchors below stand in for `{doc}` cross-references to other lectures in
the original site. They keep myst-parser quiet ("unknown document" warnings)
while preserving the surrounding prose, so the page still renders the layout
elements (headings, anchor links, inline link styling) that the theme is
being tested against.

(placeholder-stats-examples)=
**stats_examples** — placeholder for cross-reference to the companion lecture
on probability distributions and Python sampling.

(placeholder-prob-meaning)=
**prob_meaning** — placeholder for cross-reference to the lecture on the
meaning of probability.

(placeholder-navy-captain)=
**navy_captain** — placeholder for cross-reference to the navy captain
decision problem lecture.

(placeholder-lln-clt)=
**lln_clt** — placeholder for cross-reference to the law of large numbers and
central limit theorem lecture.

(placeholder-multivariate-normal)=
**multivariate_normal** — placeholder for cross-reference to the multivariate
normal distribution lecture.

(placeholder-opt-transport)=
**opt_transport** — placeholder for cross-reference to the optimal transport
lecture.
